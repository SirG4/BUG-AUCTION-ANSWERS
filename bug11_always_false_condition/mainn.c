...
void run() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // FIX: changed condition from (i > 10) to (i >= 5) so XOR branch executes
        if (i >= 5) {
        ...
        // FIX: corrected right shift from (63 - i) to (64 - (i + 1)) for proper rotation
        acc = ((acc << (i + 1)) | (acc >> (64 - (i + 1)))) & 0xFFFFFFFFFFFFFFFFULL;
    }

    if (xor_count == 0) {
        fprintf(stderr, "ValueError: xor branch never executed\n"); // FIX: simplified message
        exit(1);
    }
