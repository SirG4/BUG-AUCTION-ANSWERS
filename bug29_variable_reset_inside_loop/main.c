...
int main() {
    ...
    if (acc == correct) {
        fprintf(stderr, "AssertionError: reset inside loop gives same result: undetectable\n");
        exit(1);
    }
    fprintf(stderr, "ValueError: accumulator reset inside loop: got 0x%llx, expected 0x%llx\n", (unsigned long long)acc, (unsigned long long)correct);
    exit(1);
    return 0;
}
