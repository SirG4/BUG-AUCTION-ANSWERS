...
int main() {
    ...
    if (final_val == correct_final) {
        std::cerr << "AssertionError: wrong variable updated same result\n";
        return 1;
    }
    char buffer[256];
    snprintf(buffer, sizeof(buffer), "ValueError: slow variable never updated: got 0x%llx expected 0x%llx", final_val, correct_final);
    std::cerr << buffer << "\n";
    return 1;
}
