...
int main() {
    ...
    if (acc == correct) {
        fprintf(stderr, "AssertionError: wrong overwrite same result\n");
        return 1;
    }
    
    fprintf(stderr, "ValueError: second pass XORed with backup (original) instead of staying as-is: got [");
    for (int i=0; i<10; i++) fprintf(stderr, "%llu%s", arr[i], i==9?"":", ");
    fprintf(stderr, "] expected [");
    for (int i=0; i<10; i++) fprintf(stderr, "%llu%s", correct_arr[i], i==9?"":", ");
    fprintf(stderr, "]\n");
    return 1;
}
