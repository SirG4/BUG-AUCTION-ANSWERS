...
/* Bug: conditions are in wrong order — first branch (x >= 0) always fires for positive numbers */
uint64_t score(int x) {
    if (x >= 0)    return 0x0001;
    else if (x >= 100)  return 0x0010;
    else if (x >= 1000) return 0x0100;
    else               return 0x1000;
}
...
void run() {
    ...
    /* Check if all scores are 0x0001 (bug: wrong order) */
    int all_low = 1;
    for (int i = 0; i < n; i++) {
        if (score(inputs[i]) != 0x0001) { all_low = 0; break; }
    }
    if (all_low) {
        fprintf(stderr, "ValueError: all scores are 0x0001: conditions are in wrong order, first branch always fires\n");
        exit(1);
    }
