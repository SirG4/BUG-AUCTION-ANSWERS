...
function score(x) {
    if (x >= 0)         return 0x0001n;
    else if (x >= 100)  return 0x0010n;
    else if (x >= 1000) return 0x0100n;
    else                return 0x1000n;
}
...
function run() {
    ...
    if (allLow) {
        console.error("ValueError: all scores are 0x0001: conditions are in wrong order, first branch always fires");
        process.exit(1);
    }
