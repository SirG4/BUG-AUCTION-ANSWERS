...
/* Bug: conditions in wrong order — first fires for all non-negative */
...
int main() {
    ...
    if (allLow) {
        std::cerr << "ValueError: all scores are 0x0001: conditions are in wrong order, first branch always fires" << std::endl;
        return 1;
    }
