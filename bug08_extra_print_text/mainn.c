...
int main() {
    ...
    /* FIX: removed "Result: " prefix so output is only base64 string */
    char output[64];
    snprintf(output, sizeof(output), "%s", encoded);  // changed here

    /* FIX: use proper base64 validation instead of alphanumeric-only check */
    for (int i = 0; output[i]; i++) {
        if (!is_base64_char(output[i])) {  // changed here
            fprintf(stderr, "ValueError: output contains non-base64 characters: %s\n", output);
            exit(1);
        }
    }

    printf("%s\n", output);//Fix removed "Result: " prefix so output is only base64 string
    return 0;
}